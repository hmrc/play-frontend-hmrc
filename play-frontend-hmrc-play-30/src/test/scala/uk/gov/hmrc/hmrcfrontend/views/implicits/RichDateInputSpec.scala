...
class RichDateInputSpec extends AnyWordSpec with Matchers with MessagesSupport with GuiceOneAppPerSuite {
  ...
  "Given a DateInput object, calling withFormField" should {
    ...
    "create three InputItems if none passed in" in {
      val dateInput = DateInput().withFormField(dateField).withDayMonthYear(dateField)

      dateInput.items should have length 3
    }

    "create two InputItems for withMonthAndYearOnly" in {
      val dateInput = DateInput().withFormField(dateField).withMonthAndYearOnly(dateField)
