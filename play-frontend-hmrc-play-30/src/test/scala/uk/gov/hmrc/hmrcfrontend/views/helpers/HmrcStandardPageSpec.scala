...
class HmrcStandardPageSpec
  ...
  "HmrcStandardPage" should {
    ...
    "bind a service name in the default service navigation when passed in" in {
      ...
      val serviceNameLink = document.select(".govuk-service-navigation__service-name")
    ...
    "include the language select in default service navigation if Welsh translations flagged as available" in {
      ...
      val languageSelect = document.select(".hmrc-service-navigation-language-select")
    ...
    "include a non-linked service name if passed a service name" in {
      ...
      val homepageText = document.select(".govuk-service-navigation__text")
      homepageText          should have size 1
    ...
    "include exit this page if passed an ExitThisPage" in {
      ...
      exitThisPageButton.text()       shouldBe "Emergency Exit this page"
    ...
    "correctly translate exit this page based on language in messages" in {
      ...
      val exitThisPageButton = document.select(".govuk-exit-this-page__button")
      exitThisPageButton                should have size 1
      exitThisPageButton.text()       shouldBe "Argyfwng Gadael y dudalen hon"
      exitThisPageButton.attr("href") shouldBe "https://www.bbc.co.uk/weather"
    }
