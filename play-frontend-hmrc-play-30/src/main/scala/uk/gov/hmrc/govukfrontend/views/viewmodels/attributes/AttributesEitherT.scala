...
case class AttributesEitherT(attributes: Seq[AttributeEitherT], additionalAttributes: Map[String, String] = Map.empty) {
  override def toString(): String = {
    val combinedAttributes = attributes ++ additionalAttributes.toSeq.map { case (k, v) =>
      AttributeEitherT(k, EitherT.left(Some(v)))
    }
    combinedAttributes.foldLeft("")((currentAttr, newAttr) => s"$currentAttr ${newAttr.toString}".trim)
  }
}

case class AttributeEitherT(name: String, value: EitherT[Option, String, Boolean], isOptional: Boolean = false) {
