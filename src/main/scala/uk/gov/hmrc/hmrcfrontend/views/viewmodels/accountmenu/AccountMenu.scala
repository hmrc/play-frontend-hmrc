...
import uk.gov.hmrc.hmrcfrontend.views.viewmodels.accountmenu.Language.{En, LanguageFormat}
...
object AccountMenu extends JsonDefaultValueFormatter[AccountMenu] {
  ...
  override implicit def jsonWrites: OWrites[AccountMenu] =
    (
      (__ \ "accountHome").write[AccountHome] and
      (__ \ "messages").write[Messages] and
      (__ \ "checkProgress").write[CheckProgress] and
      (__ \ "paperlessSettings").write[PaperlessSettings] and
      (__ \ "personalDetails").write[PersonalDetails] and
      (__ \ "signOut").write[SignOut] and
      (__ \ "language").write[Language](LanguageFormat)
    )(unlift(AccountMenu.unapply))
}
